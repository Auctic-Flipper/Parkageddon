...
# ============================================================================
SERVER_URL = "http://localhost:3000/test"  # Your server endpoint
API_KEY = ""  # Optional: if your server requires authentication
CAMERA_LOCATION = "Main Garage"  # Identifier for your parking location
ENABLE_HTTP_POSTING = False  # Set to False to disable HTTP posts for testing
# ============================================================================

# Camera configurationsbbbb
CAMERAS = [
    {
        'name': 'Camera 1 (107)',
        'ip': '192.168.1.107',
...
class HTTPNotifier:
    """Handles HTTP POST requests to update server with count changes"""
    ...
    def _posting_worker(self):
        """Background worker that sends POST requests"""
        while True:
            try:
    ...
    def _send_post_request(self, payload):
        ...
        if not ENABLE_HTTP_POSTING:
            print(f"[{self.camera_name}] HTTP posting disabled - would send: {payload}")
            return
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"[{self.camera_name}] ✓ Server updated: {payload['count']} vehicles")
            else:
                self.failed_posts += 1
                print(f"[{self.camera_name}] ✗ Server error {response.status_code}: {response.text[:100]}")
                
        except requests.exceptions.Timeout:
            self.failed_posts += 1
            print(f"[{self.camera_name}] ✗ Server timeout")
        except requests.exceptions.ConnectionError:
            self.failed_posts += 1
            print(f"[{self.camera_name}] ✗ Connection error - is server running?")
        except Exception as e:
            self.failed_posts += 1
            print(f"[{self.camera_name}] ✗ POST error: {e}")
    
    def notify_count_change(self, count, change_type, track_id):
        """Queue a count change notification"""
        payload = {
    ...
    def get_stats(self):
        ...
        return {
            ...
            'pending': self.post_queue.qsize()
        }
    
    def shutdown(self):
        """Gracefully shutdown the notifier"""
        self.post_queue.put(None)  # Shutdown signal
        self.posting_thread.join(timeout=2)
...
class CameraProcessor:
    ...
    def __init__(self, camera_config, camera_index):
        ...
        # Initialize HTTP notifier
        self.http_notifier = HTTPNotifier(
    ...
    def update_line_crossing(self, track_id, current_x, current_y):
        ...
        if not state['counted']:
            ...
            if (state['line1_crossed'] and state['line2_crossed'] and 
                ...
                # **NEW: Send HTTP notification**
                self.http_notifier.notify_count_change(
    ...
    def process_frame(self, frame):
        ...
        # **NEW: HTTP posting stats**
        http_stats = self.http_notifier.get_stats()
        http_status = f"HTTP: ✓{http_stats['successful']} ✗{http_stats['failed']} ⏳{http_stats['pending']}"
        http_color = (0, 255, 0) if http_stats['failed'] == 0 else (0, 165, 255)
        cv2.putText(frame, http_status, (10, 260),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, http_color, 2)
    ...
    def run(self):
        ...
        # Show HTTP posting status
        if ENABLE_HTTP_POSTING:
            print(f"[{self.name}] ✓ HTTP posting ENABLED to {SERVER_URL}")
        else:
            print(f"[{self.name}] ⚠ HTTP posting DISABLED (test mode)")
        ...
        # Shutdown HTTP notifier
        print(f"[{self.name}] Shutting down HTTP notifier...")
        self.http_notifier.shutdown()
        ...
        http_stats = self.http_notifier.get_stats()
        print(f"  HTTP Posts: ✓{http_stats['successful']} ✗{http_stats['failed']}")
...
if __name__ == "__main__":
    print("\n" + "="*70)
    print("MULTI-CAMERA VEHICLE COUNTER WITH HTTP NOTIFICATIONS")
    print("="*70)
    print(f"Number of cameras: {len(CAMERAS)}")
    for i, cam in enumerate(CAMERAS):
        print(f"  Camera {i+1}: {cam['name']} ({cam['ip']}) -> {cam.get('location_id', 'N/A')}")
    print(f"\nServer URL: {SERVER_URL}")
    print(f"HTTP Posting: {'ENABLED' if ENABLE_HTTP_POSTING else 'DISABLED (test mode)'}")
    print("="*70 + "\n")
    ...
    print(f"HTTP notifications: {'ENABLED' if ENABLE_HTTP_POSTING else 'DISABLED'}")
